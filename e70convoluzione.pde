...
float[][] convoluzione(PImage I, float[][] K)
{
  ...
  //i due cicli for che ciclano su x e y
  ...
  for(int x=0; x<I.width; x++)
  {
    for(int y=0; y<I.height;y++)
    {
      ...
      //i due cicli su p ed n ciclano sul kernel
      for(int p=0; p<N; p++)
      {
        for(int q=0; q<M;q++)
        {
...
PImage converti(float[][] F)
{
  ...
  for(int x = 0; x < R.width; x++)
  {
    for(int y = 0; y < R.height; y++)
    {
      ...
      //questa riga di codice fà una sorta di normalizzazione,
      //perchè costringe il valore, prima reso positivo, tra 0 e 255
